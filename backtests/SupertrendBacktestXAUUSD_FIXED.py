...
class SupertrendTickEngine:
    ...
    def run(self, tick_df: pd.DataFrame, bars_df: pd.DataFrame) -> pd.DataFrame:
        ...
        print("Dynamically consolidating ticks into M1 bounds for hybrid exit evaluation...")
        t0_m1 = time.time()
        m1_df = tick_df.resample('1min').agg({
            'Bid': ['min', 'max', 'mean'],
            'Ask': ['min', 'max', 'mean']
        }).dropna()
        m1_df.columns = ['_'.join(col).strip() for col in m1_df.columns.values]
        m1_df['Spread_mean'] = m1_df['Ask_mean'] - m1_df['Bid_mean']
        
        m1_ns = m1_df.index.values.astype('datetime64[ns]').astype(np.int64)
        m1_bids_lo = (m1_df['Bid_min'] - m1_df['Spread_mean']).values
        m1_asks_hi = (m1_df['Ask_max'] + m1_df['Spread_mean']).values
        m1_spreads = m1_df['Spread_mean'].values
        num_m1 = len(m1_ns)
        
        bar_m1_starts = np.searchsorted(m1_ns, bar_ns, side='left')
        bar_m1_ends = np.append(bar_m1_starts[1:], num_m1)
        print(f"M1 consolidation completed in {time.time() - t0_m1:.2f} seconds.")

        tick_restricted = np.zeros(num_ticks, dtype=bool)
        if self.cfg.enable_market_hours:
            ...
            tick_times = pd.Series(pd.to_datetime(tick_ns)).dt.time.values
            if close_cutoff > open_cutoff:
                tick_restricted = (tick_times >= close_cutoff) | (tick_times < open_cutoff)
            else:
                tick_restricted = (tick_times >= close_cutoff) & (tick_times < open_cutoff)
        ...
        for idx in range(1, n_bars):
            ...
            bar_restr = tick_restricted[start_t:end_t]
            ...
            bar_m1_bids_lo = m1_bids_lo[m1_start:m1_end]
            bar_m1_asks_hi = m1_asks_hi[m1_start:m1_end]
            bar_m1_ns = m1_ns[m1_start:m1_end]
            bar_m1_spreads = m1_spreads[m1_start:m1_end]
            ...
            if in_position:
                ...
                # Check SL using M1 bounds (Hybrid)
                if pos_type == "long":
                    m1_sl_hits = np.where(bar_m1_bids_lo <= current_sl)[0]
                else:
                    m1_sl_hits = np.where(bar_m1_asks_hi >= current_sl)[0]
                    
                if pending_flip_exit["type"] is not None:
                    exit_level = pending_flip_exit["level"]
                    m1_f_hits = np.where(bar_m1_bids_lo <= exit_level)[0] if pos_type == "long" else np.where(bar_m1_asks_hi >= exit_level)[0]
                    if len(m1_f_hits) > 0 and (len(m1_sl_hits) == 0 or m1_f_hits[0] <= m1_sl_hits[0]):
                        m1_sl_hits = m1_f_hits
                        exit_reason = "SuperTrend Flip Exit"
                    elif len(m1_sl_hits) > 0:
                        exit_reason = "SuperTrend SL Hit"
                elif len(m1_sl_hits) > 0:
                    exit_reason = "SuperTrend SL Hit"
                
                # If an M1 exit is triggered, find the exact tick!
                if len(m1_sl_hits) > 0:
                    m1_hit_idx = m1_sl_hits[0]
                    m1_hit_time_ns = bar_m1_ns[m1_hit_idx]
                    minute_end_ns = m1_hit_time_ns + 60000000000
                    
                    tick_sub_start = np.searchsorted(tick_ns[start_t:end_t], m1_hit_time_ns, side='left')
                    tick_sub_end = np.searchsorted(tick_ns[start_t:end_t], minute_end_ns, side='left')
                    
                    if tick_sub_start == tick_sub_end:
                        exit_idx = tick_sub_start
                    else:
                        sub_bids = bar_bids[tick_sub_start:tick_sub_end]
                        sub_asks = bar_asks[tick_sub_start:tick_sub_end]
                        if pos_type == "long":
                            tick_hits = np.where(sub_bids <= current_sl)[0]
                        else:
                            tick_hits = np.where(sub_asks >= current_sl)[0]
                            
                        if len(tick_hits) > 0:
                            exit_idx = tick_sub_start + tick_hits[0]
                        else:
                            exit_idx = tick_sub_start
                            
                    # Pass the M1 average spread up to fallback filter
                    m1_avg_spread = bar_m1_spreads[m1_hit_idx]
                ...
                if self.cfg.enable_market_hours and self.cfg.market_buffer_mode == "close":
                    ...
                    if len(restr_hits) > 0:
                        ...
                        if exit_idx == -1 or restr_hit_idx < exit_idx:
                            exit_idx = restr_hit_idx
                ...
                if exit_idx != -1:
                    ...
                    if exit_reason in ["SuperTrend SL Hit", "SuperTrend Flip Exit"]:
                        tick_spread = exit_ask - exit_bid
                        if tick_spread > m1_avg_spread * 2.0:
                            if pos_type == "long":
                                exit_bid = exit_ask - m1_avg_spread
                            else:
                                exit_ask = exit_bid + m1_avg_spread
            ...
            if not in_position:
                ...
                if pending_sl_reentry["type"] is not None:
                    ...
                    if re_type != curr_tr_str:
                        ...
                        m1_hits = np.where(bar_m1_asks_hi >= re_level)[0] if re_type == "long" else np.where(bar_m1_bids_lo <= re_level)[0]
                        if len(m1_hits) > 0:
                            for m1_hit_idx in m1_hits:
                                m1_hit_time_ns = bar_m1_ns[m1_hit_idx]
                                minute_end_ns = m1_hit_time_ns + 60000000000
                                tick_sub_start = np.searchsorted(tick_ns[start_t:end_t], m1_hit_time_ns, side='left')
                                tick_sub_end = np.searchsorted(tick_ns[start_t:end_t], minute_end_ns, side='left')
                                if tick_sub_start < tick_sub_end:
                                    sub_asks = bar_asks[tick_sub_start:tick_sub_end]
                                    sub_bids = bar_bids[tick_sub_start:tick_sub_end]
                                    tick_hits = np.where(sub_asks >= re_level)[0] if re_type == "long" else np.where(sub_bids <= re_level)[0]
                                    if self.cfg.enable_market_hours:
                                        sub_restr = bar_restr[tick_sub_start:tick_sub_end]
                                        tick_hits = tick_hits[~sub_restr[tick_hits]]
                                    if len(tick_hits) > 0:
                                        for th in tick_hits:
                                            idx_cand = tick_sub_start + th
                                            c_ask = bar_asks[idx_cand]
                                            c_bid = bar_bids[idx_cand]
                                            if (c_ask - c_bid) > self.cfg.max_spread_dollars:
                                                continue # Ignore tick if spread is too high
                                            
                                            # We found a valid tick! Check Monday filter.
                                            cand_ts = pd.Timestamp(tick_ns[start_t:end_t][idx_cand])
                                            if cand_ts.weekday() == 0:
                                                ...
                                                if 0 <= mins_since_open < self.cfg.disable_monday_mins:
                                                    # Cancel signal completely (MT5 Virtual Stop logic)
                                                    pending_sl_reentry = {"type": None, "level": 0.0, "signal_time": None, "high": 0.0, "low": 0.0}
                                                    break
                                            
                                            # Check Max Gap / Slippage (Virtual Stop)
                                            synthetic_price = c_ask if re_type == "long" else c_bid
                                            if abs(synthetic_price - re_level) > self.cfg.max_slippage_dollars:
                                                # Cancel signal completely
                                                pending_sl_reentry = {"type": None, "level": 0.0, "signal_time": None, "high": 0.0, "low": 0.0}
                                                break
                                                
                                            entry_hit_idx = idx_cand
                                            entry_m1_idx = m1_hit_idx
                                            break
                                        if entry_hit_idx != -1 or pending_sl_reentry["type"] is None:
                                            break
                        if entry_hit_idx != -1:
                ...
                if entry_hit_idx == -1 and pending_breakout["type"] is not None:
                    ...
                    if b_type != curr_tr_str:
                        ...
                        m1_hits = np.where(bar_m1_asks_hi >= b_level)[0] if b_type == "long" else np.where(bar_m1_bids_lo <= b_level)[0]
                        if len(m1_hits) > 0:
                            for m1_hit_idx in m1_hits:
                                m1_hit_time_ns = bar_m1_ns[m1_hit_idx]
                                minute_end_ns = m1_hit_time_ns + 60000000000
                                ...
                                if tick_sub_start < tick_sub_end:
                                    sub_asks = bar_asks[tick_sub_start:tick_sub_end]
                                    sub_bids = bar_bids[tick_sub_start:tick_sub_end]
                                    tick_hits = np.where(sub_asks >= b_level)[0] if b_type == "long" else np.where(sub_bids <= b_level)[0]
                                    if self.cfg.enable_market_hours:
                                        sub_restr = bar_restr[tick_sub_start:tick_sub_end]
                                        tick_hits = tick_hits[~sub_restr[tick_hits]]
                                    if len(tick_hits) > 0:
                                        for th in tick_hits:
                                            ...
                                            # Check Max Gap / Slippage (Virtual Stop)
                                            synthetic_price = c_ask if b_type == "long" else c_bid
                                            if abs(synthetic_price - b_level) > self.cfg.max_slippage_dollars:
                                                # Cancel signal completely
                                                pending_breakout = {"type": None, "level": 0.0, "signal_time": None, "high": 0.0, "low": 0.0}
                                                break
                                            ...
                                            entry_m1_idx = m1_hit_idx
                                            break
                                        if entry_hit_idx != -1 or pending_breakout["type"] is None:
                                            break
                        if entry_hit_idx != -1:
                ...
                if entry_hit_idx != -1:
                    ...
                    # Apply fake spread filter for entries based on M1 average spread
                    tick_spread = entry_ask - entry_bid
                    m1_avg_spread = bar_m1_spreads[entry_m1_idx]
                    if tick_spread > m1_avg_spread * 2.0:
                        if pos_type == "long":
                            entry_ask = entry_bid + m1_avg_spread
                        else:
                            entry_bid = entry_ask - m1_avg_spread
